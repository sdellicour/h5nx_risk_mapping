...
if (savingPlots)
	{
		...
		for (i in 1:length(envVariableNames))
			{
				envVariableName = envVariableNames[i]; substr(envVariableName,1,1) = toupper(substr(envVariableName,1,1))				
				envVariables[[i]] = raster(paste0("Environmental_data/Prepared_rasters/",envVariableName,".tif"))
				if (grepl("population_density",envVariableName))
					{
						envVariables[[i]][] = log(envVariables[[i]][]+1)
						envVariableNames[i] = paste0(envVariableNames[i],"_(log)")
					}
			}
		...
		i1s = c(1,29,57); i2s = c(28,56,length(envVariables))
		for (h in 1:3)
			{		
				pdf(paste0("Env_variables_",h,".pdf"), width=14, height=14) # dev.new(width=14, height=14)
				par(mfrow=c(7,4), oma=c(0,0,0,0), mar=c(0,0,0,0), lwd=0.2, col="gray30"); options(scipen=9)
				for (i in i1s[h]:i2s[h])
					{
						plot(envVariables[[i]], col=colourScale, ann=F, axes=F, box=F, legend=F)
						plot(coast_lines, lwd=0.1, col="gray30", add=T)
						envVariableName = gsub("_"," ",envVariableNames[i])
						substr(envVariableName,1,1) = toupper(substr(envVariableName,1,1))				
						mtext(envVariableName, line=-1.8, cex=0.8, col="gray30")
						plot(envVariables[[i]], legend.only=T, add=T, col=colourScale, legend.width=0.5, alpha=1, legend.shrink=0.3, smallplot=c(0.35,0.87,0.14,0.17),
...
for (i in 1:length(variables_sets))
	{
		envVariableNames = data_sources[,"variable_name_2"]
		envVariableNames = envVariableNames[which(data_sources[,"kind_of_variable"]==variables_sets[i])]
		envVariableNames = envVariableNames[envVariableNames!=""]
		envVariableNames = envVariableNames[!envVariableNames%in%c("distance_to_water","vaccination_in_china")]
		envVariables = list()
		for (j in 1:length(envVariableNames))
			{		
				envVariableName = envVariableNames[j]; substr(envVariableName,1,1) = toupper(substr(envVariableName,1,1))				
				envVariables[[j]] = raster(paste0("Environmental_data/Prepared_rasters/",envVariableName,".tif"))
			}

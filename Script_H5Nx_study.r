...
if (savingPlots)
	{
		...
		i1s = c(1,37); i2s = c(36,length(allEnvVariables))
		for (h in 1:length(i1s))
			{		
				...
				for (i in i1s[h]:i2s[h])
					{
						plot(allEnvVariables[[i]], col=colourScale, ann=F, axes=F, box=F, legend=F)
						plot(coast_lines, lwd=0.1, col="gray30", add=T)
						envVariableName = gsub("_"," ",allEnvVariableNames[i])
						substr(envVariableName,1,1) = toupper(substr(envVariableName,1,1))				
						mtext(envVariableName, line=-1.8, cex=0.8, col="gray30")
						plot(allEnvVariables[[i]], legend.only=T, add=T, col=colourScale, legend.width=0.5, alpha=1, legend.shrink=0.3, smallplot=c(0.35,0.87,0.14,0.17),
...
serotype_summary = table(tab[,"serotype"]); serotypes = c("H5N1","H5")
...
if (!file.exists("BRT_dataframes.rds"))
	{
		...
		human_pop_density_log = human_pop_density; human_pop_density_log[] = log(human_pop_density_log[]+1)
		for (i in 1:length(datasets))
			{
				dataframe = cbind(rep(1,dim(datasets[[i]])[1]), datasets[[i]][,"longitude"], datasets[[i]][,"latitude"])
				PAs = seegSDM::bgSample(human_pop_density_log, n=10*dim(datasets[[i]])[1], prob=T, replace=F, spatial=F)
				...
				for (j in 1:length(minimumDistances))
					{
						minimumDistances[j] = min(rdist.earth(cbind(PAs[j,1],PAs[j,2]), cbind(datasets[[i]][,"longitude"],datasets[[i]][,"latitude"]), miles=F))
					}
				...
				dataframe = rbind(dataframe, cbind(rep(0,dim(PAs)[1]),PAs)); colnames(dataframe) = c("response","longitude","latitude")
...
# 4. Boosted regression trees (BRT) analyses with standard or spatial cross-validations
